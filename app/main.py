...
# # def get_df():
# #     data = pd.read_csv('artifacts/data.csv')
# #     data1 = data.drop('blood_glucose', axis=1)
# #     return data1

# # def add_sidebar():
# #     with st.sidebar:
# #         selected = option_menu('Non Invasive Glucometer: Diabeters Prediction System',['Diabetes prediction'],icons= ['activity','heart','person'], default_index = 0)
# #     return selected
...
# #         st.title("Non Invasive Glucometer")
# #         st.subheader("AI-based Prediction Powered by Patient Biometrics & Lifestyle Data")
# #         st.markdown("##### Please connect this app to your glucometer to help diagnose blood sugar level from your test parameters and improve the accuracy. This app predicts the blood sugar level based on diagonistic measures.")
...
#                 # Commented out model prediction logic
#                 # if submitted:
...
#                 # 📊 Summary Stats Table
#             _, df = get_df()
#             selected_cols = ['age', 'blood_glucose', 'blood_pressure_systolic', 'blood_pressure_diastolic', 'bmi']  # Numerical features only
#             st.subheader("📈 Visualisation")
#             # Melt only selected columns
#             df_melted = df[selected_cols].melt(var_name="Feature", value_name="Value")
#             # Add a dropdown for feature selection
...
#                 # Add interpretation and health tips
#                 st.subheader("Insights & Recommendations")
#                 avg_values = df[feature_options].mean().round(2)
#                 st.write("**Average Values in Training Data:**", avg_values.to_dict())
#                 for feature in feature_options:
#                     if feature == 'bmi' and avg_values[feature] > 25:
#                         st.write(f"- **High BMI Alert**: Your BMI ({bmi:.1f}) is above the average ({avg_values[feature]}). Consider consulting a doctor and increasing physical activity.")
#                     elif feature == 'blood_glucose' and avg_values[feature] > 126:
#                         st.write(f"- **Elevated Glucose Alert**: The average blood glucose ({avg_values[feature]} mg/dL) is above normal (126 mg/dL). Monitor levels closely.")
#                     elif feature == 'age' and avg_values[feature] > 45:
#                         st.write(f"- **Age Consideration**: The average age ({avg_values[feature]}) is a risk factor. Regular check-ups are recommended.")
...
#         #             # 📊 Summary Stats Table
#         #         _,df = get_df()
#         #         selected_cols = ['age','blood_glucose','blood_pressure_systolic','blood_pressure_diastolic', 'bmi', 'socioeconomic_status', 'BMI_Category']
#         #         st.subheader("📈 Visualisation")
#         #         # Melt only selected columns
#         #         df_melted = df[selected_cols].reset_index().melt(id_vars="index",var_name="Feature", value_name="Value")
#         #         # Plot using Plotly
#         #         import plotly.express as px
#         #         fig_bar = px.line(
#         #             df_melted, x=df_melted.index, y="Value", color="Feature",
#         #             # title="Key Feature Distribution Across Patients",
#         #             labels={"index": "Sample Index", "Value": "Feature Value"}
#         #         )
